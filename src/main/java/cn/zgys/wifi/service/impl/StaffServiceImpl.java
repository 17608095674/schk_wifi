	...
	public String login(String userName, String password) {
		...
		if (userName.trim().matches(pattern)) {
			...
			//2、比较过期时间和当前时间，如果已经过期则需要重新注册
				...
				log.setIdCard(staff.getIdCard());
				log.setRealName(staff.getRealName());
				log.setArea("区域一");
			...
			try {
				...
				result = stub.validatorLegalUser(userName, password);
			} catch (AxisFault e) {
...
//			cn.schk.oa.WFInterfaceEntity staff = soap.getInterfaceEntity2("WIFI", "WIFI", "123", System.currentTimeMillis()+Code.getCard());
	...
	public String regist1(String realName, String phoneNumber, String code, String idCard, String name, String account, String limit, String day, String reason) {
		...
		sta.setReason(reason);
		sta.setCreateTime(new Date());
		sta.setBusiness(name + "(" + account + ")");
		sta.setLit("互联网");
		...
		for (EmployeeBean employeeBean : list) {
			...
			entity.setFormURL("http://172.18.81.246:80/approveOut?&phoneNumber=" + phoneNumber + "&day=" + day +
...
//			entity.setUserNameConsigned("");//委托人姓名（选填）
	...
	public String regist2(String realName, String phoneNumber, String code, String idCard, String name, String account, String limit, String day, String reason) {
		...
		for (EmployeeBean employeeBean : list) {
			...
			entity.setFormURL("http://172.18.81.246:80/approve1?phoneNumber=" + phoneNumber + "&day=" + day +
...
//	}	
	...
	public String check(String phoneNumber, String day) {
		...
		if ("1".equals(result)) {
			...
			H3cUtil.addUser(phoneNumber, pwd);
			//生成密码，将更新的信息保存到数据库中
...
//	}
	...
	@Override
	public String ref(String phoneNumber, String step) {
		System.err.println("步骤：" + step);
		//根据手机号找到该用户的审批流水号
		Staff staff = staffRepository.findByPhoneNumber(phoneNumber);
		//该手机号码为注册页面传入过来，而在注册时就已经生成了instanceId，所以staff.getInstanceId()不可能为空！
		String instanceId = staff.getInstanceId();
		...
		OaWfWebService oa = new OaWfWebService();
		OaWfWebServiceSoap soap = (OaWfWebServiceSoap) oa.getOaWfWebServiceSoap();
		String result = soap.updateWFInstanceStatus("WIFI", "WIFI", "123", instanceId, step, null, null, time, time);
		if ("1".equals(result)) {
			SMS2009 sms = new SMS2009();
			SMS2009Soap smsSoap = sms.getSMS2009Soap();
			String xmlCommand = "<Root>\r\n" +
	...
	@Override
	public String approve1(String phoneNumber, String day, String apa1, String date1) {
		// TODO Auto-generated method stub
		...
		System.err.println("流水号：" + instanceId);
		//更新staff信息
		staff.setStatus(2);
		...
		for (EmployeeBean employeeBean : list) {
			...
			entity.setProjectName("WIFI");
			entity.setInstanceTitle("内网无线网络申请");
			entity.setFormURL("http://172.18.81246:80/approve2?phoneNumber=" + phoneNumber + "&apa1=" + apa1 + "&day=" +
					day + "&date1=" + date1 + "&apa2=" + employeeBean.getAccount());
			//处理相关事项的表单页面的地址,连接外网。
			entity.setStepName("领导审批");//步骤名称
			entity.setFlowType("WIFI");//流转步骤的类型
			entity.setImportLevel("0");//缓急程度
...
//			entity.setUserNameConsigned("");//委托人姓名（选填）
			...
			entity.setCreatName(employeeBeanRepository.findByAccountEquals(apa1).getName());//撰稿人姓名
			//获取当前时间
			entity.setCreatTime(df.format(new Date()));
			entity.setCreatDeptName("信息服务部");//撰稿人所在部门名称
	...
	@Override
	public String approve2(String phoneNumber, String day) {
		// TODO Auto-generated method stub
		//根据手机号找到该用户的审批流水号
		Staff staff = staffRepository.findByPhoneNumber(phoneNumber);
		//该手机号码为注册页面传入过来，而在注册时就已经生成了instanceId，所以staff.getInstanceId()不可能为空！
	...
	@Override
	public Staff staff(String phoneNumber) {
